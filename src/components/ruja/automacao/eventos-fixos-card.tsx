...
export default function EventosFixosCard() {
  ...
  const [form, setForm] = useState({ nome: '', tipo: 'Reunião', recorrencia: 'semanal' as Recorrencia, dia_semana: 6, dia_mes: 1, hora_inicio: '10:00', hora_termino: '', local: '', departamento_id: '' })

  const carregar = useCallback(async () => {
    try { setLista(await fetchEventosFixos()); setDepartamentos(await fetchDepartamentos()) }
  ...
  async function handleCriar() {
    ...
    try {
      await criarEventoFixo({ nome: form.nome, tipo: form.tipo, recorrencia: form.recorrencia, dia_semana: form.recorrencia === 'semanal' ? form.dia_semana : null, dia_mes: form.recorrencia === 'mensal' ? form.dia_mes : null, hora_inicio: form.hora_inicio, hora_termino: form.hora_termino || null, local: form.local || null, descricao: null, departamento_id: form.departamento_id || null, departamentos_envolvidos: form.departamento_id ? [] : ['Todos'], lider_responsavel_id: null })
  ...
  return <Card icon={CalendarDays} title="Eventos Fixos" subtitle="Se repetem sozinhos toda semana ou todo mês -- o Paulo cria a ocorrência no dia certo, o líder só lança a frequência.">
    <div className="space-y-2 mb-3">
      ...
      {lista.map(ev => <div key={ev.id} className="flex items-center justify-between bg-black/30 rounded-lg px-3 py-2"><div><p className="text-white text-sm font-medium">{ev.nome}</p><p className="text-gray-500 text-xs">{ev.recorrencia === 'semanal' ? DIAS_SEMANA[ev.dia_semana ?? 0] : `Dia ${ev.dia_mes} do mês`} às {ev.hora_inicio.slice(0, 5)}{ev.local ? ` · ${ev.local}` : ''}</p></div><div className="flex items-center gap-2"><button onClick={() => alternarEventoFixo(ev.id, !ev.ativo).then(carregar)} className={`px-2 py-1 rounded-full text-xs font-semibold ${ev.ativo ? 'bg-green-500/15 text-green-400' : 'bg-white/5 text-gray-500'}`}>{ev.ativo ? 'Ativo' : 'Pausado'}</button><button onClick={() => excluirEventoFixo(ev.id).then(carregar)} className="text-gray-600 hover:text-red-400 text-xs"><X size={14} /></button></div></div>)}
    </div>
    ...
    {!aberto ? <button onClick={() => setAberto(true)} className="w-full py-2.5 bg-white/5 hover:bg-white/10 text-gray-300 text-sm rounded-xl">+ Novo evento fixo</button> : <div className="space-y-2 bg-black/30 rounded-lg p-3">
      ...
      <input placeholder="Local (opcional)" value={form.local} onChange={e => setForm({ ...form, local: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white text-sm" /><select value={form.departamento_id} onChange={e => setForm({ ...form, departamento_id: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white text-sm"><option value="">Todos os departamentos</option>{departamentos.map(d => <option key={d.id} value={d.id}>{d.nome}</option>)}</select>
