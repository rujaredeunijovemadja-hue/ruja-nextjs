'use client'
// ─── ANALISTA IA DO RUJA ──────────────────────────────────────
// IA que analisa dados reais do Supabase (frequência, jovens,
// metas, recuperação, departamentos, aniversários, crescimento).
// Regra: apenas leitura. Nunca altera dados.
// Usa a mesma API Anthropic já configurada no projeto.
...
// ── Monta o contexto real do banco para o system prompt ───────
function montarContexto(state: ReturnType<typeof useRuja>): string {
  ...
  const jovensResumo = jovens.map(j => {
    ...
    return `- ${j.nome} | Depto: ${j.departamento || 'Sem depto'} | Status: ${status} | Frequência: ${pct}% | Faltas seguidas: ${faltas} | Batizado: ${j.batizado} | Aniversário em: ${diasAniv === 0 ? 'HOJE' : `${diasAniv} dias`}`
  }).join('\n')
  ...
  const historicoResumo = historicoMensal
    .slice(-6)
    .map(h => `- ${h.mes}: Total ${h.total} | Ativos em depto: ${h.ativos_depto} | Batizados: ${h.batizados_depto}`)
    .join('\n')
  ...
  return `
SISTEMA: Você é o Analista IA do RUJA (Rede UniJovem ADJA), sistema de gestão da juventude da Igreja ADJA.
Você analisa DADOS REAIS do banco de dados e responde com precisão e empatia pastoral.

REGRAS ABSOLUTAS:
✅ Pode: analisar, explicar, resumir, identificar tendências, gerar alertas, comparar períodos.
❌ NUNCA pode: alterar dados, excluir registros, registrar presenças, alterar jovens, líderes ou permissões.
❌ NUNCA invente números. Se não houver dados, diga claramente.

Responda sempre em português, com linguagem pastoral e objetiva.
Use emojis moderadamente para facilitar leitura.
Organize respostas com seções quando necessário.

=== DADOS ATUAIS DA JUVENTUDE ===

📊 VISÃO GERAL (${new Date().toLocaleDateString('pt-BR')}):
- Total de jovens: ${totalJovens}
- Ativos: ${ativos} | Oscilando: ${oscilando} | Em Risco: ${emRisco} | Ociosos: ${ociosos}
- Batizados: ${batizados} (${totalJovens ? Math.round(batizados/totalJovens*100) : 0}%)
- Aniversariantes nos próximos 7 dias: ${aniversNaSemana}
- Total de frequências registradas: ${frequencias.length}
- Em recuperação ativa: ${recuperacoes.filter(r => r.status === 'ativo').length}

🎯 METAS:
- Jovens ativos em departamento: ${ativos} / ${metas.ativosDepto} (meta)
- Batizados ativos: ${batizados} / ${metas.batizadosDepto} (meta)

📋 REGRAS DE STATUS:
- Ativo: frequência ≥ ${regras.ativo}%
- Oscilando: frequência ≥ ${regras.oscilando}%
- Em Risco: ${regras.risco} ou mais faltas seguidas
- Ocioso: abaixo de ${regras.oscilando}%

👥 JOVENS (${totalJovens} total):
${jovensResumo || 'Nenhum jovem cadastrado.'}

🏛️ DEPARTAMENTOS (${departamentos.length} total):
${deptosResumo || 'Nenhum departamento cadastrado.'}

🚑 RECUPERAÇÃO ATIVA:
${recuperacaoResumo || 'Nenhum jovem em recuperação ativa.'}

📈 HISTÓRICO MENSAL (últimos 6 meses):
${historicoResumo || 'Sem histórico mensal disponível.'}

⭐ LÍDERES (${lideres.length} total):
${lideres.map(l => `- ${l.nome} | ${l.funcao} | Depto: ${l.departamento}`).join('\n') || 'Nenhum líder cadastrado.'}
`.trim()
}
...
export default function RujaAnalistaIA() {
  ...
  return (
    <div className="flex flex-col h-full max-h-[calc(100vh-80px)] md:max-h-screen">
      {/* ── HEADER ─────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-4 md:px-6 pt-4 pb-3 border-b border-white/8 flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 space-y-4">
          ...
          <div className="space-y-5">
            <div className="text-center pt-4">
              ...
              <p className="text-gray-500 text-sm mt-1 max-w-xs mx-auto">
                Analiso dados reais da sua juventude. Pergunte qualquer coisa — frequência,
                alertas, metas, aniversários, crescimento.
              </p>
            </div>
            ...
            <div className="bg-blue-500/8 border border-blue-500/20 rounded-xl px-4 py-3 flex items-start gap-3">
              <span className="text-blue-400 text-base flex-shrink-0">ℹ️</span>
        ...
        {mensagens.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && (
              <div className="w-7 h-7 bg-red-500/15 rounded-lg flex items-center justify-center text-sm flex-shrink-0 mt-0.5 mr-2">
                🦁
              </div>
            )}
        ...
        {pensando && (
          <div className="flex items-start gap-2">
            <div className="w-7 h-7 bg-red-500/15 rounded-lg flex items-center justify-center text-sm flex-shrink-0 mt-0.5">
              🦁
            </div>
      ...
      {/* ── INPUT ──────────────────────────────────────────── */}
      <div className="px-4 md:px-6 pb-4 pt-2 border-t border-white/8 flex-shrink-0">
        <div className="flex gap-2 items-end">
          ...
          <button
            ...
            className="w-11 h-11 bg-red-600 hover:bg-red-700 disabled:opacity-40 rounded-xl flex items-center justify-center flex-shrink-0 transition touch-manipulation"
          >
            {pensando ? <Spinner size="sm" /> : <span className="text-white text-base">↑</span>}
          </button>
        </div>
...
// ── Formata a resposta da IA com markdown básico ──────────────
    ...
    <div className="space-y-1">
      {linhas.map((linha, i) => {
        ...
        if (linha.startsWith('**') && linha.endsWith('**')) return (
          <p key={i} className="font-semibold text-white">{linha.slice(2, -2)}</p>
        )
