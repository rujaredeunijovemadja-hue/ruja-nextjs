...
export function RujaJovemDetalhe({ jovem, onClose, onEdit }: Props) {
  const { frequencias, recuperacoes } = useRuja()

  const pct         = getFreqPct(jovem.id, frequencias)
  const faltasSeg   = getFaltasSeguidas(jovem.id, frequencias)
      ...
      <div className="bg-[#111] border border-white/10 rounded-t-2xl md:rounded-2xl w-full max-w-lg max-h-[92dvh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/8 flex-shrink-0">
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            <button onClick={onClose}
              className="p-2 text-gray-400 hover:text-white text-xl touch-manipulation">✕</button>
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          <div className="flex gap-2">
            {jovem.contato && (
              <a href={`https://wa.me/55${jovem.contato.replace(/\D/g,'')}`}
                target="_blank" rel="noreferrer"
                className="flex-1 flex items-center justify-center gap-2 bg-green-500/10 border border-green-500/20 rounded-xl py-2.5 text-green-400 text-sm touch-manipulation">
                📱 WhatsApp
              </a>
            )}
            {jovem.instagram && (
              <a href={`https://instagram.com/${jovem.instagram.replace('@','')}`}
                target="_blank" rel="noreferrer"
                className="flex-1 flex items-center justify-center gap-2 bg-purple-500/10 border border-purple-500/20 rounded-xl py-2.5 text-purple-400 text-sm touch-manipulation">
                📸 Instagram
              </a>
            )}
          </div>
          ...
          <div className="bg-black/30 rounded-xl p-4">
            <div className="text-gray-400 text-xs uppercase tracking-wider mb-3">Frequência</div>
            <div className="flex items-center gap-4">
              <div className="text-center">
                <div className={`text-2xl font-black ${pct >= 75 ? 'text-green-400' : pct >= 40 ? 'text-yellow-400' : 'text-red-400'}`}>
                  {pct}%
                </div>
              ...
              <div className="flex-1 bg-white/5 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${pct >= 75 ? 'bg-green-500' : pct >= 40 ? 'bg-yellow-500' : 'bg-red-500'}`}
                  style={{ width: `${pct}%` }}
                />
              </div>
              <div className="text-center">
            ...
            {ultFreqs.length > 0 && (
              <div className="mt-3 pt-3 border-t border-white/5">
                <div className="text-gray-500 text-xs mb-2">Últimas presenças</div>
                <div className="flex flex-wrap gap-1.5">
                  {ultFreqs.map(f => (
                    <div key={f.id}
                      className={`text-xs px-2 py-1 rounded-lg ${f.presenca === 'presente' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            {deptos.length > 0 && (
              <InfoItem label="Departamentos" value={deptos.join(', ')} />
            )}
            {jovem.lider && <InfoItem label="Líder" value={jovem.lider} />}
            {jovem.data_nasc && (
              <InfoItem
                label="Aniversário"
                value={`${jovem.data_nasc.slice(8,10)}/${jovem.data_nasc.slice(5,7)} · ${idade} anos${diasAniv === 0 ? ' 🎂' : diasAniv <= 7 ? ` · em ${diasAniv}d` : ''}`}
              />
            )}
